import axiosClient from '@core/axiosClient'
import { useCallback, useState } from 'react'

import { getBackendUrl } from '@shared/utils/env'
import { type ApiState } from '@shared/hooks/types'

export type GetTenantSubscriptionExpiryStatusResponse = {
  subscription_plan_name: string;
  days_until_expiration: number;
}

export const useGetTenantSubscriptionExpiryStatus = <T = GetTenantSubscriptionExpiryStatusResponse>() => {
  const [state, setState] = useState<ApiState<T>>({
    data: null,
    loading: false,
    error: null
  })

  const getTenantSubscriptionExpiryStatus = useCallback(async (tenantId: string): Promise<T> => {
    setState(prevState => ({ ...prevState, loading: true, error: null }))

    const url = `${getBackendUrl()}/api/v1/tenant/${tenantId}/subscription-expiry-status`

    try {
      const response = await axiosClient.get<T>(url.replace(getBackendUrl(), ''))
      setState({ data: response.data as T, loading: false, error: null })
      return response.data as T
    } catch (error: any) {
      setState({ data: null, loading: false, error: new Error(error.message) })
      throw error
    }
  }, [setState])

  return { ...state, getTenantSubscriptionExpiryStatus }
}
