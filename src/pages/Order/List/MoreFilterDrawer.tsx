...
export const MoreFilterDrawer: React.FC<MoreFilterDrawerProps> = ({ open, onClose, onApplyFilters, initialFilters }) => {
  ...
  const [startDatetime, setStartDatetime] = useState<Dayjs | null>(
    initialFilters?.start_datetime ? dayjs(initialFilters.start_datetime) : null
  );
  const [endDatetime, setEndDatetime] = useState<Dayjs | null>(
    initialFilters?.end_datetime ? dayjs(initialFilters.end_datetime) : null
  );

  useEffect(() => {
    if (open) {
      setStoreIds(initialFilters?.store_ids || []);
      setStartDatetime(initialFilters?.start_datetime ? dayjs(initialFilters.start_datetime) : null);
      setEndDatetime(initialFilters?.end_datetime ? dayjs(initialFilters.end_datetime) : null);
    }
  }, [open, initialFilters]);
